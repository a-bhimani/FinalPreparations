...
public class LargestSmallerBstKey{
  ...
  static class BinarySearchTree{
    ...
    int findLargestSmallerKey(int num){
      // your code goes here
      return findMaxWithin(this.root, num);
    }

    private int findMaxWithin(Node n, int num){
      int max = -1;
      ...
      if(n.key < num){
        max = n.key;
        max = Math.max(max, findMaxWithin(n.right, num));
      }

      max = Math.max(max, findMaxWithin(n.left, num));
